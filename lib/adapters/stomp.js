...
function wildcardName(queue, pattern) {
  if (pattern == null || pattern === '') {
    return queue;
  }
...
function stompSubscribe(self, pattern, callback) {
  var wildcard = wildcardName(self._q, pattern);
  ...
  c(self)._doWhenReady(function pullSubscribe(done) {
    ...
    console.log('subscribe to', wildcard);

    s(self).subscribe(wildcard, null, function(msg, headers) {
...
function stompPublish(self, msg, queue) {
  if (queue == null) {
    queue = self._q;
  }
  ...
  c(self)._doWhenReady(function pushPublish(done) {
    ...
    s(self).publish(queue, encoding[0], headers);
